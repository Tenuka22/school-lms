...
#[derive(Debug, Deserialize, JsonSchema, ApiComponent)]
pub struct SaveWorkspaceAddressesRequest {
    pub addresses: Vec<AddressEntry>,
}

#[derive(Debug, Serialize, JsonSchema, ApiComponent)]
pub struct SaveWorkspaceAddressesResponse {
    pub count: usize,
}

#[api_operation(tag = "g1-applications", operation_id = "save-workspace-addresses")]
pub async fn save_workspace_addresses(
    ...
    info!("[save_workspace_addresses] deleted {deleted:?} existing join rows for app {app_id}");

    let mut count = 0;
    for entry in &body.addresses {
        info!(
            "[save_workspace_addresses] inserting join workspace_address_id={} address_type={} residence_type={} is_primary={}",
            entry.workspace_address_id, entry.address_type, entry.residence_type, entry.is_primary
        );
        join_workspace_addresses::ActiveModel {
            id: Set(Uuid::new_v4()),
            application_id: Set(app_id),
            workspace_address_id: Set(entry.workspace_address_id),
    ...
    info!("[save_workspace_addresses] success count={}", count);
    ...
    crate::audit::log_application_change(
        ...
        None,
        Some(serde_json::json!({"workspace_address_count": count})),
        &auth,
        Some(format!("workspace addresses saved: {} linked", count)),
    )
    .await;

    Ok(web::Json(SaveWorkspaceAddressesResponse { count }))
}
