...
use crate::entity::g1::join_guardians;
...
#[async_trait]
impl<'a> JoinGuardiansRepo for SeaOrmJoinGuardiansRepo<'a> {
    async fn find_by_application(&self, app_id: Uuid) -> Result<Vec<join_guardians::Model>, DbErr> {
        use sea_orm::{ColumnTrait, EntityTrait, QueryFilter};
        join_guardians::Entity::find()
    ...
    async fn delete_by_application(&self, app_id: Uuid) -> Result<(), DbErr> {
        use sea_orm::{ColumnTrait, EntityTrait, QueryFilter};
        join_guardians::Entity::delete_many()
            .filter(join_guardians::Column::ApplicationId.eq(app_id))
            .exec(self.db)
            .await?;
        Ok(())
    ...
    async fn insert_many(&self, models: Vec<join_guardians::Model>) -> Result<Vec<join_guardians::Model>, DbErr> {
        ...
        for model in models {
            let active: join_guardians::ActiveModel = model.into();
