...
import { getApplicationOptions, getApplicationQueryKey, getApplicationGuardiansOptions, getApplicationWorkspaceAddressesOptions, getApplicationSiblingsOptions, getApplicationDocumentsOptions, listApplicationsQueryKey, updateApplicationMutation, saveWizardStepMutation, saveGuardiansMutation, saveWorkspaceAddressesMutation, saveSiblingsMutation, saveApplicationDocumentsMutation, listWorkspaceAddressesOptions } from "@/lib/api-client/@tanstack/react-query.gen"
import { queryClient } from "@/router"
import type { Gender, Nationality, MediumOfInstruction, G1Category, Religion } from "@/lib/api-client/types.gen"
...
export function WizardShell() {
  const params = useParams({ from: "/_authenticated/student-management/enrollment/g1/$enrollment_id" })
  ...
  const { data: applicationGuardianIds } = useQuery(
    getApplicationGuardiansOptions({ path: { id: enrollmentId }, client: apiClient }),
  )

  const { data: applicationAddressEntries } = useQuery(
    getApplicationWorkspaceAddressesOptions({ path: { id: enrollmentId }, client: apiClient }),
  )

  const { data: applicationSiblingIds } = useQuery(
    getApplicationSiblingsOptions({ path: { id: enrollmentId }, client: apiClient }),
  )

  const { data: serverDocuments } = useQuery(
    getApplicationDocumentsOptions({ path: { id: enrollmentId }, client: apiClient }),
  )
  ...
  const saveSiblings = useMutation(
    saveSiblingsMutation({ client: apiClient }),
  )
  ...
  const autoSaveStep = useCallback(async (stepNum: number) => {
    await saveStep.mutateAsync({
      path: { id: enrollmentId },
      body: { wizard_step: stepNum },
    })
    setSavedSteps((prev) => Math.max(prev, stepNum))
    queryClient.invalidateQueries({ queryKey: getApplicationQueryKey({ path: { id: enrollmentId }, client: apiClient }) })
  }, [enrollmentId, saveStep])

  const autoSaveChild = useCallback(async (data: ChildFormData) => {
    try {
  ...
  const selectedGuardians = (guardians ?? []).filter((g) => guardianIds.includes(g.id))
          ...
          return (
            <div key={label} className="flex items-center gap-2 shrink-0">
                ...
                className={`flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-full border transition-colors ${
                  isActive
                    ...
                    : isLocked
                      ...
                      : isCompleted
                        ? "border-green-500 text-green-600 cursor-pointer hover:bg-green-50 dark:hover:bg-green-950/20"
              ...
              >
                ...
                >
                  {isCompleted ? "\u2713" : isActive ? stepNum : isLocked ? "\u2715" : stepNum}
                </span>
                {label}
              </button>
              {i < STEPS.length - 1 && (
